...
In order to run this under the root account, you will need to add the following parameter to `/etc/chromium/default` between the quotes for `CHROMIUM_FLAGS=â€â€`
...
Install the appropiate VirtualBox Extension Pack on to the host. These packs can be found here ([https://www.virtualbox.org/wiki/Downloads](https://www.virtualbox.org/wiki/Downloads)) for the most recent,  
